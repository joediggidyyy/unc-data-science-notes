...
<img src="codesentinel\assets\CodeSentinelLogo.png" alt="CodeSentinel Logo" display="block" width="250" style="float: left;">
</br>
...
CodeSentinel is a cross-platform automated maintenance and security monitoring system that integrates seamlessly with development workflows. It provides intelligent repository maintenance, security scanning, and multi-channel alerting to keep your codebase healthy, secure, and SEAM-tight.</br></br></br></br>
...
[![Version](https://img.shields.io/badge/version-1.1.5-green.svg)](https://github.com/joediggidyyy/CodeSentinel)
...
When working on multi-phase initiatives (for example Phase 1 → Phase 2 → Phase 3), follow the repository's phase-deterministic moniker policy for filenames, logs, questframes, and reports. This makes artifacts trivially discoverable for new contributors and automation. See `docs/policies/governance/GOVERNANCE_POLICY/PP_GOV_PROTOCOL_POL_PHASE_NAMING_20251127.md` for canonical patterns and examples.

Health reports
--------------

We collect end-of-job and end-of-day health reports exclusively under `logs/health_reports/`, organized into scope-specific subfolders (for example `logs/health_reports/QS-A/PHASE3_HEALTH_20251127.{json,md}`). Use `tools/oracode/scripts/generate_health_report.py` to generate the canonical JSON artifact and its derived markdown.
...
- **Unified data streams** - Behavioral telemetry, SessionMemory tiers, QuestStack exports, and health reports feed a single awareness bus so gaps raise an immediate SEAM flag.
- **One-touch policy access** - Machine-readable policy indices, policy snapshots, and governance lookups stay online 24/7 so agents never act without context.
- **Calculated value guardianship** - Complexity estimators, ORACode priorities, flash manifests, and remediation scoring remain synchronized with live evidence.
- **Ecosystem fail-fast** - If any awareness feed drifts or lags, CodeSentinel halts risky workflows until instrumentation is healthy again.
...
```text
# Copy into a local .env (do not commit) and fill values via your secrets manager.
...
CodeSentinel is **SEAM Protected™** (Security, Efficiency, Awareness, Minimalism) with security as the highest priority:
...
# Provision the canonical repository environment (.venv-core, Python 3.14)
python -m codesentinel.cli venv ensure --primary --requirements requirements.txt
.\.venv-core\Scripts\activate  # Linux/macOS: source .venv-core/bin/activate

# Keep the venv staging area compliant (dry run is optional but recommended)
py -3 tools\codesentinel\venv_stage_manager.py plan --include-managed
py -3 tools\codesentinel\venv_stage_manager.py cleanup --dry-run

# Provision a managed dev environment (reuses if it already exists)
python -m codesentinel.cli venv ensure --name dev --requirements requirements-dev.txt
...
- **SEAM Protection™**: Security, Efficiency, Awareness, and Minimalism guide all decisions
...
- **Primary Environment**: `.venv-core/` pinned to Python 3.14.x (no TTL, never pruned)
- **Sandboxes**: `.venv-*` purpose-built environments for testing/automation with TTL governance
- **Python**: 3.13 or higher for ancillary tooling (3.14 recommended)
...
- **[Primary Venv Policy](docs/guides/PRIMARY_VENV_POLICY.md)** - Canonical vs sandbox environment rules
...
- **SEAM Protection™**: Security, Efficiency, Awareness, and Minimalism
